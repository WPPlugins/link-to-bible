...
function ltb_show_post($content) {
	$options = ltb_get_options ();
	global $post;
	if (! get_post_meta ( $post->ID, 'LTB_DISABLE', true ) && (! get_post_meta ( $post->ID, '_ltb_last', true ) || ltb_notnull_and_unequal ( get_post_meta ( $post->ID, '_ltb_translation', true ), ltb_get_bible_version ( $options, $post ) ))) {
...
function ltb_notnull_and_unequal($s1, $s2) {
	return $s1 && $s2 && $s1 != $s2;
}
...
function ltb_save_post($content) {
	...
	if (! get_post_meta ( $post->ID, 'LTB_DISABLE', true )) {
		...
		delete_post_meta ( $post->ID, '_ltb_lang' );
		return $content;
	} else {
		return $content;
	}
...
function ltb_add_links($content, $post, $options, $ignore_errors = false) {
	...
	if (! $result) {
		if (! $ignore_errors) {
			set_transient ( ltb_get_transient_hash (), 'Link-To-Bible Error: Error while connecting to bibleserver.com', 10 );
		}
		return $content;
	}
...
function ltb_mark_to_ignore_false_positive($options, $content) {
	if (! isset ( $options ['ignore_false_positive'] ) && $options ['ignore_false_positive'])
		return $content;
	$patterns = array (
...
function ltb_check_for_options_update($options) {
	...
	if (isset ( $options ['aak_on'] ) && $options ['aak_on'] && ($options ['aak_domain'] != get_option ( 'siteurl' ))) {
		$apikey = ltb_retrieve_apikey ();
		if ($apikey) {
			$options ['apikey'] = $apikey;
			$options ['aak_domain'] = get_option ( 'siteurl' );
	...
	// Set ltb-version
	if (! isset ( $options ['ltbver'] ) || $options ['ltbver'] != $GLOBALS ['LTB_VERSION']) {
		$options ['ltbver'] = $GLOBALS ['LTB_VERSION'];
		update_option ( 'ltb_options', $options );
	}
	// Set bible-language
	if (! isset ( $options ['biblelang'] )) {
...
function ltb_get_locale() {
	...
	$locale = isset ( $GLOBALS ['LTBLANG'] ) ? $GLOBALS ['LTBLANG'] : null;
	if (empty ( $locale )) {
		$locale = get_locale ();
	}
	if (empty ( $locale )) {
...
function ltb_get_language_for_bible_version($searched_version_key, $options) {
	if (isset ( $options ['refformatlang'] ) && $options ['refformatlang']) {
		foreach ( ltb_get_masterdata () as $lang => $lang_data ) {
			foreach ( $lang_data ['bible_versions'] as $version_key => $version_name ) {
...
function ltb_validate_options($input) {
	if (isset ( $input ['aak_on'] ) && $input ['aak_on'] && (! isset ( $input ['apikey'] ) || ! $input ['apikey'])) {
		$apikey = ltb_retrieve_apikey ();
		if ($apikey) {
			$input ['apikey'] = $apikey;
...
function ltb_options_page() {
	...
	if ((! isset ( $options ['aak_on'] ) || (! $options ['aak_on']))) {
	...
	jQuery(document).ready(function($) {
		...
		jQuery("#langsel").on('change keyup', function() {
			...
			jQuery.getJSON('<?php print plugin_dir_url ( __FILE__ ) . "resources/bibleversions.json" ?>', function(data) {
				...
				var $curlang = "<?php print isset($options['translation']) ? $options['translation'] : ""; ?>";
				$bversel.empty();
...
<div class="wrap">
	...
	<form action="options.php" id="ltb_options_form" method="post">
			...
			<tr>
				...
				<td><p>
						<input type="checkbox" id="ltb_aak_cb" name="ltb_options[aak_on]" value="1"
							<?php checked( 1 == ((isset($options['aak_on']) ? $options['aak_on'] : 0))); ?> /> <?php _e("Retrieve API-Key automatically", "ltb")?>
					</p>
					<p>
						<input type="text" id="ltb_apikey_inp" size="60" name="ltb_options[apikey]"
							value="<?php echo isset($options['apikey']) ? $options['apikey'] : ""; ?>" />
					</p>
			...
			<tr>
				...
				<td><input type="checkbox" name="ltb_options[ignore_false_positive]" value="1"
					<?php checked( 1 == (isset($options['ignore_false_positive']) ? $options['ignore_false_positive'] : 0) ); ?> /> <?php _e("Ignore False-Positives", "ltb")?>
						<p class="description"><?php _e('Some statements are detected by bibleserver.com as bible references which are no ones.', 'ltb') ?></p>
					<br> <input type="checkbox" name="ltb_options[refformatlang]" value="1"
					<?php checked( 1 == (isset($options['refformatlang']) ? $options['refformatlang'] : 0) ); ?> /> <?php _e("Use the language of a post's bible version for detecting bible references", "ltb")?>
...
function ltb_http_post_request($url, $params) {
	if (function_exists ( 'curl_init' )) {
		...
		curl_setopt ( $ch, CURLOPT_USERAGENT, "LinkToBible/" . $GLOBALS ['LTB_VERSION'] );
		$result = curl_exec ( $ch );
		$errno = curl_errno ( $ch );
		$error = curl_error ( $ch );
		curl_close ( $ch );
		...
		$http = array (
				'http' => array (
						...
						'user_agent' => "LinkToBible/" . $GLOBALS ['LTB_VERSION'],
